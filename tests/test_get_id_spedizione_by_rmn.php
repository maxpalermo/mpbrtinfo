<?php
/**
 * Script di test per la classe GetIdSpedizioneByRMN
 * 
 * Questo script permette di testare la funzionalità di ricerca dell'ID spedizione BRT
 * tramite riferimento mittente numerico utilizzando la classe GetIdSpedizioneByRMN.
 * 
 * Utilizzo: 
 * - Da browser: /modules/mpbrtinfo/tests/test_get_id_spedizione_by_rmn.php?riferimento=RIFERIMENTO&cliente_id=ID_CLIENTE
 * - Da CLI: php test_get_id_spedizione_by_rmn.php RIFERIMENTO ID_CLIENTE
 */
...
if (empty($riferimento) || empty($cliente_id)) {
    if (php_sapi_name() === 'cli') {
        echo "Utilizzo: php test_get_id_spedizione_by_rmn.php RIFERIMENTO ID_CLIENTE\n";
    } else {
        echo '<!DOCTYPE html>
<html>
<head>
    ...
    <title>Test GetIdSpedizioneByRMN</title>
...
<body>
    <div class="container">
        <h1 class="mb-4">Test GetIdSpedizioneByRMN</h1>
        <div class="card">
...
try {
    ...
    if ($result === false) {
        ...
        if (php_sapi_name() !== 'cli') {
            ...
            echo '<label for="riferimento">Riferimento Mittente Numerico:</label>';
            echo '<input type="text" class="form-control" id="riferimento" name="riferimento" value="' . htmlspecialchars($riferimento) . '" required>';
            echo '</div>';
            echo '<div class="form-group">';
