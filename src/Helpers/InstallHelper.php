...
class InstallHelper
{
    ...
    public static function createOrderState(
        ...
        $is_shipped = false,
        $is_paid = false,
        $has_pdf_invoice = false,
        $has_pdf_delivery = false,
        $icon = ''
    ) {
        ...
        foreach ($states as $state) {
            if (preg_match('/' . $name . '/i', $state['name'])) {
                $state_exist = true;
        ...
        if (!$state_exist) {
            ...
            $order_state->module_name = 'mpbrtinfo';
            ...
            $order_state->is_hidden = $is_hidden;
            $order_state->logable = $is_logable;
            $order_state->delivered = $is_delivered;
            $order_state->shipped = $is_shipped;
            $order_state->paid = $is_paid;
            $order_state->pdf_invoice = $has_pdf_invoice;
            $order_state->pdf_delivery = $has_pdf_delivery;

            try {
                $res_os = $order_state->add();
                if ($res_os) {
                    if ($icon) {
                        $id_order_state = (int) $order_state->id;
                        $module = \Module::getInstanceByName($module_name);
                        $source = $module->getLocalPath() . 'views/img/gif/' . $icon . '.gif';
                        $dest = _PS_IMG_DIR_ . 'os/' . $id_order_state . '.gif';
                ...
                \PrestaShopLogger::addLog($th->getMessage(), 1, $th->getCode(), 'MpBrtInfo');
            }
        }
