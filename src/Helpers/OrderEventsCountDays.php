...
class OrderEventsCountDays
{
     ...
     */
    public static function get($orderId, $dateShipped = '', $dateDelivered = '')
    {
        ...
        $orderId = (int) $orderId;
        $dateShipped = pSQL($dateShipped);
        $dateDelivered = pSQL($dateDelivered);

        if (!$orderId) {
            return false;
        }

        if ($dateShipped && (\Validate::isDateFormat($dateShipped)) && $dateDelivered && (\Validate::isDateFormat($dateDelivered))) {
            $days = self::countDays($dateShipped, $dateDelivered);
    ...
    public static function countDays($date_start, $date_end)
    {
        ...
        if ($date_start) {
            $date_start = self::formatYMD($date_start);
        }
        if ($date_end) {
            $date_end = self::formatYMD($date_end);
        }
