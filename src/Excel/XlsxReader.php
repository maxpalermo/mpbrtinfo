 ...
 *
 * Example 1:
 * if ( $xlsx = XlsxReader::parse('book.xlsx') ) {
 *   print_r( $xlsx->rows() );
 * } else {
 ...
 *
 * Example 3: rowsEx
 * $xlsx = XlsxReader::parse('book.xlsx');
 * print_r( $xlsx->rowsEx() );
 *
 ...
 *
 * Example 8: read data
 * if ( $xlsx = XlsxReader::parseData( file_get_contents('http://www.example.com/example.xlsx') ) ) {
...
namespace MpSoft\MpBrtInfo\Excel;
    ...
    */
    public function getEntryXML($name)
    {
        if ($entry_xml = $this->getEntryData($name)) {
            ...
            // }
            $entry_xml_obj = simplexml_load_string($entry_xml);
    ...
    // https://github.com/shuchkin/XlsxReader#gets-extend-cell-info-by--rowsex
