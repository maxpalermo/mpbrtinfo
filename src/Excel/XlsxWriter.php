...
class XlsxWriter
{
    ...
    protected $template;
    protected $F;
    protected $F_KEYS; // fonts
    protected $XF;
    protected $XF_KEYS; // cellXfs
    protected $SI;
    protected $SI_KEYS; // shared strings
    public const N_NORMAL = 0; // General
    public const N_INT = 1; // 0
    public const N_DEC = 2; // 0.00
    public const N_PERCENT_INT = 9; // 0%
    public const N_PERCENT_DEC = 10; // 0.00%
    public const N_DATE = 14; // mm-dd-yy
    public const N_TIME = 20; // h:mm
    public const N_DATETIME = 22; // m/d/yy h:mm
    public const F_NORMAL = 0;
    public const F_HYPERLINK = 1;
    public const F_BOLD = 2;
    public const F_ITALIC = 4;
    public const F_UNDERLINE = 8;
    public const F_STRIKE = 16;
    public const A_DEFAULT = 0;
    public const A_LEFT = 1;
    public const A_RIGHT = 2;
    public const A_CENTER = 3;

    public function __construct()
    {
        $smarty = \Context::getContext()->smarty;
        $tpl_path = dirname(__FILE__) . '/views/templates/admin/excel/_partials/';
        $smarty->assign('__CLASS__', __CLASS__);
        ...
        $this->template = [
            '_rels/.rels' => $smarty->fetch($tpl_path . 'rels.tpl'),
            'docProps/app.xml' => $smarty->fetch($tpl_path . 'app.xml.tpl'),
            'docProps/core.xml' => $smarty->fetch($tpl_path . 'core.xml.tpl'),
            'xl/_rels/workbook.xml.rels' => $smarty->fetch($tpl_path . 'workbook.xml.rels.tpl'),
            'xl/worksheets/sheet1.xml' => $smarty->fetch($tpl_path . 'sheet1.xml.tpl'),
            'xl/worksheets/_rels/sheet1.xml.rels' => $smarty->fetch($tpl_path . 'sheet1.xml.rels.tpl'),
            'xl/sharedStrings.xml' => $smarty->fetch($tpl_path . 'sharedStrings.xml.tpl'),
            'xl/styles.xml' => $smarty->fetch($tpl_path . 'styles.xml.tpl'),
            'xl/workbook.xml' => $smarty->fetch($tpl_path . 'workbook.xml.tpl'),
            '[Content_Types].xml' => $smarty->fetch($tpl_path . 'content_types.xml.tpl'),
        ];
    }
    ...
    public function addSheet(array $rows, $name = null)
    {
        ...
        if ($name === null) { // autogenerated sheet names
            ...
            sheets[$this->curSheet] = ['name' => $name, 'hyperlinks' => []];
                ...
                foreach ($this->sheets as $k => $v) {
                    if (count($v['hyperlinks'])) {
                        ...
                        foreach ($v['hyperlinks'] as $h) {
                            $RH[] = '
<Relationship Id="' . $h['ID'] . '" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/hyperlink"
    Target="' . $this->esc($h['H']) . '" TargetMode="External" />';
                        }
...
<Override PartName="/_rels/.rels" ContentType="application/vnd.openxmlformats-package.relationships+xml" />'];
                foreach ($this->sheets as $k => $v) {
                    $TYPES[] = '
<Override PartName="/xl/worksheets/sheet' . ($k + 1) .
                                                '.xml"
    ContentType="application/vnd.openxmlformats-officedocument.spreadsheetml.worksheet+xml" />';
                    if (count($v['hyperlinks'])) {
                        $TYPES[] = '
<Override PartName="/xl/worksheets/_rels/sheet' . ($k + 1) . '.xml.rels"
    ...
    protected function _writeEntry($fh, &$cdrec, $cfilename, $data)
    {
        ...
        $e['crc_32'] = crc32($data); // Convert date and time to DOS Format, and set then
    ...
    protected function _sheetToXML($idx, $template)
    {
        ...
        if (count($this->sheets[$idx]['rows'])) {
                ...
                foreach ($r as $v) {
                    ...
                    if (is_string($v)) {
                        if ($v[0] === "\0") { // RAW value as string
                            ...
                            if (strpos($v, '<') !== false) {
                                // tags? if (strpos($v, '<b>' ) !==false) { $F +=self::F_BOLD; }
                                if (strpos($v, '<i>' ) !== false) {
                                    $F += self::F_ITALIC;
                                }
                                if (strpos($v, '<u>' ) !== false) {
                                ...
                                if (preg_match('/<a href="(https?:\/\/[^"]+)">(.*?)<\ /a>/i', $v, $m)) {
                                    $h = explode('#', $m[1]);
                                    $this->sheets[$idx]['hyperlinks'][] = ['ID' => 'rId' . (count($this->sheets[$idx]['hyperlinks']) +
                                    1), 'R' => $cname, 'H' => $h[0], 'L' => isset($h[1]) ? $h[1] : ''];
                                    $F = self::F_HYPERLINK; // Hyperlink
                                }
                                if (preg_match('/<a href="(mailto?:[^" ]+)">(.*?)<\ /a>/i', $v, $m)) {
                                    $this->sheets[$idx]['hyperlinks'][] = ['ID' => 'rId' .
                                    (count($this->sheets[$idx]['hyperlinks']) + 1), 'R' => $cname, 'H' => $m[1], 'L' => ''];
                            ...
                            if ($v === '0' || preg_match('/^[-+]?[1-9]\d{0,14}$/', $v)) { // Integer as General
                                ...
                                $h = explode('#', $v);
                                $this->sheets[$idx]['hyperlinks'][] = ['ID' => 'rId' .
                                (count($this->sheets[$idx]['hyperlinks']) + 1), 'R' => $cname, 'H' => $h[0], 'L' => isset($h[1]) ? $h[1] : ''];
                                $F = self::F_HYPERLINK; // Hyperlink
                            } elseif (preg_match(
                                ...
                                $this->sheets[$idx]['hyperlinks'][] = ['ID' => 'rId' .
                                (count($this->sheets[$idx]['hyperlinks']) + 1), 'R' => $cname, 'H' => 'mailto:' . $v,
                                    'L' => ''];
                                $F = self::F_HYPERLINK; // Hyperlink
                            }
                        }
            ...
            foreach ($COL as $k => $max) {
                $COLS[] = '
        <col min="' . $k . '" max="' . $k . '" width="' . min($max + 1, 60) . '" />';
            }
            $COLS[] = '
    </cols>';
        ...
        if (count($this->sheets[$idx]['hyperlinks'])) {
            ...
            foreach ($this->sheets[$idx]['hyperlinks'] as $h) {
                $HYPERLINKS[] = '
        <hyperlink ref="' . $h['R'] . '" r:id="' . $h['ID'] . '" location="' . $this->esc($h['L']) . '"
            display="' . $this->esc($h['H'] . ($h['L'] ? ' - ' . $h['L'] : '')) . '" />';
            }
            $HYPERLINKS[] = '
    </hyperlinks>';
        }
    ...
    public function date2excel($year, $month, $day, $hours = 0, $minutes = 0, $seconds = 0)
    {
        ...
        if (((int) $year === 1900) && ($month <= 2)) {
            $excel1900isLeapYear = false;
        }
        $myExcelBaseDate = 2415020; // Julian base date Adjustment
        if ($month > 2) {
            $month -= 3;
        } else {
            $month += 9;
        ...
        $excelDate = floor((146097 * $century) / 4) + floor((1461 * $decade) / 4) + floor((153 * $month + 2) / 5) + $day
        + 1721119 - $myExcelBaseDate + $excel1900isLeapYear;
